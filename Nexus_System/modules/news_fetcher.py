...
#---------------------- HTML AND LINK TEXT SANTISATION -----------------------#
def clean_html(text):
    if not text:
        return ""
...
# -------------------------- CACHE INPUT AND OUTPUT HELPERS --------------#
...
def save_cache(CACHED_FILE, LINKS):
    with open(CACHED_FILE, "w", encoding="utf-8") as f:
        f.write("\n".join(LINKS))

#----------------------------- DATE EVALUATION --------------------#
def evaluate_article(pub_date_text, max_hours=2):
...
def rss_propagator(source_name, rss_url, seen_links, BLOCKLIST):
    ...
    try:
        ...
        for item in root.findall(".//item")[:10]:
                        ...
                        if evaluate_article(pub_date_text):
...
#------------------------------- THE BEATING HEART OF THE SCRIPT -------------------#
...
def check_headlines():
    ...
    try:
        ...
        if all_new_stories:
            for i, story in enumerate(all_new_stories):
                send_discord_alert("news", story)
                if i < len(all_new_stories) - 1:
                    time.sleep(5)

            save_cache(CACHED_FILE, seen_links)
            commit_github(CACHED_FILE, f"Update news cache - {len(seen_links)} headlines tracked")
        else:
            print("No new headlines were found!")
